...
class SphereArc(ThreeDScene):
    def construct(self):
        ...
        def get_those_damn_points(masspos, p0, d_angle, howmanyoneachside):
            ...
            theline = Line(masspos, p0).set_opacity(1)
        ...
        ptest1, ptest2 = get_those_damn_points(np.array([3,-1/2,3]), np.array([3, 3,-2]), PI/20, 5)
        for pt in ptest1:
            self.play(Create(Sphere(pt, radius=0.1)))


        for pt in ptest2:
            self.play(Create(Sphere(pt, radius=0.1).set_color(YELLOW)))
